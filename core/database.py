...
class UserDataset(Base):
    """User ka CSV data permanently store karo"""
    __tablename__ = 'user_datasets'
    id          = Column(Integer, primary_key=True)
    user_id     = Column(Integer, nullable=False, index=True)
    filename    = Column(String(255), nullable=False)
    csv_data    = Column(Text, nullable=False)  # CSV as string
    columns     = Column(Text, nullable=True)   # JSON columns list
    row_count   = Column(Integer, default=0)
    uploaded_at = Column(DateTime, default=datetime.utcnow)


class UserChart(Base):
    """User ke charts permanently store karo"""
