...
class DataScienceAgent:
    ...
    def load_data(self, fp):
        try:
            ...
            # Save dataset to DB (keep last 5)
            try:
                import json as _j
                from core.database import SessionLocal as _SL, UserDataset as _UD
                _csv = open(fp, 'r', encoding='latin1').read()
                _db = _SL()
                old_ds = _db.query(_UD).filter_by(user_id=self._current_user_id if hasattr(self,'_current_user_id') else 0).order_by(_UD.uploaded_at.asc()).all()
                if len(old_ds) >= 5:
                    for o in old_ds[:len(old_ds)-4]: _db.delete(o)
                _db.add(_UD(
                    user_id=self._current_user_id if hasattr(self,'_current_user_id') else 0,
                    filename=os.path.basename(fp),
                    csv_data=_csv[:500000],
                    columns=_j.dumps(self.available_columns),
                    row_count=len(self.df)
                ))
                _db.commit(); _db.close()
            except Exception as _de:
                print(f"⚠️ Dataset DB save: {_de}")
    ...
    def generate_plot(self, plot_type="bar", column=None, groupby=None, color_scheme="default"):
        ...
        try:
            ...
            # Multiple charts support - numbered
            import glob
            existing = glob.glob('/tmp/plot_*.png')
            chart_num = len(existing) + 1
            plot_path = f'/tmp/plot_{chart_num}.png'
            # Also save as latest for backward compat
            latest_path = '/tmp/plot.png'

            if "bar" in plot_type.lower() or "histogram" in plot_type.lower():
                # Smart: use requested column if given, else best auto-detect
                use_col = column if (column and column in self.df.columns) else None

                if "histogram" in plot_type.lower():
                    hist_col = use_col if use_col and use_col in num_cols else (num_cols[0] if num_cols else None)
                    if hist_col:
                        ax.hist(self.df[hist_col].dropna(), bins=25, color=colors[0], edgecolor='white', alpha=0.85)
                        ax.set_title(f'Distribution of {hist_col}', fontsize=14, fontweight='bold')
                        ax.set_xlabel(hist_col); ax.set_ylabel('Frequency')
                elif use_col and use_col in self.df.columns:
                    if self.df[use_col].dtype == object:
                        # Categorical column → value counts bar
                        counts = self.df[use_col].value_counts().head(10)
                        bars = ax.bar(range(len(counts)), counts.values,
                                     color=colors[:len(counts)] if len(colors)>=len(counts) else colors*5)
                        ax.set_xticks(range(len(counts)))
                        ax.set_xticklabels(counts.index, rotation=45, ha='right', fontsize=9)
                        ax.set_title(f'Top 10 — {use_col}', fontsize=14, fontweight='bold')
                        ax.set_ylabel('Count')
                        ...
                        # Numeric column → show top 10 values
                        id_cols = [c for c in self.df.columns if c not in num_cols]
                        top = self.df.nlargest(10, use_col)
                        if id_cols:
                            labels = top[id_cols[0]].astype(str).values
                        else:
                            labels = [f"#{i+1}" for i in range(len(top))]
                        bars = ax.bar(range(len(top)), top[use_col].values,
                                     color=colors[:len(top)] if len(colors)>=len(top) else colors*5)
                        ax.set_xticks(range(len(top)))
                        ax.set_xticklabels(labels, rotation=45, ha='right', fontsize=9)
                        ax.set_title(f'Top 10 by {use_col}', fontsize=14, fontweight='bold')
                        ax.set_ylabel(use_col)
                elif cat_cols:
                    col = cat_cols[0]
                    counts = self.df[col].value_counts().head(10)
                    bars = ax.bar(range(len(counts)), counts.values, color=colors[0])
                    ax.set_xticks(range(len(counts)))
            ...
            plt.savefig(latest_path, dpi=72, bbox_inches='tight')
            plt.close()

            # ✅ Chart DB mein save karo permanently
            try:
                import base64, json
                ...
                chart = UserChart(
                    user_id=self._current_user_id if hasattr(self,'_current_user_id') else 0,
                    chart_title=f"{plot_type.title()} Chart — {column or target_col}",
                    image_data=img_b64
                )
                db.add(chart)
                db.commit()
...
- Hinglish mein baat kar"""
    ...
    def query(self, q, user_id=None):
        ...
        # ── Smart chart detection with column extraction ─────────────
        if any(x in q for x in ['bar chart','bar graph','barchart','column chart']):
            col = self.find_column(q)
            return self.generate_plot('bar', column=col)
        if 'histogram' in q or ('distribution' in q and 'show' in q):
            col = self.find_column(q)
            return self.generate_plot('histogram', column=col)
        if 'scatter' in q or 'correlation plot' in q:
            return self.generate_plot('scatter')
        if 'line chart' in q or 'line graph' in q or 'trend chart' in q:
            col = self.find_column(q)
            return self.generate_plot('line', column=col)
        if 'pie chart' in q or 'pie graph' in q:
            col = self.find_column(q)
            return self.generate_plot('pie', column=col)
