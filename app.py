...
def payment_page():
    ...
    plan     = request.args.get('plan', 'hourly')
    amount   = request.args.get('amount', '500')
    duration = request.args.get('duration', '1 Hour Pro Access')
...
@app.route('/api/payment/qr_config')
def payment_qr_config():
    """Return QR code - supports base64, URL, or file"""
    import base64 as _b64, os as _os

    qr_data = ''

    # Priority 1: Base64 stored in env (most reliable!)
    qr_b64 = _os.getenv('QR_CODE_B64', '')
    if qr_b64:
        qr_data = 'data:image/png;base64,' + qr_b64

    # Priority 2: QR image file in static folder
    ...
    # Priority 3: URL (least reliable - CORS issues)
    elif _os.getenv('QR_CODE_URL', ''):
        qr_data = _os.getenv('QR_CODE_URL', '')

    return jsonify({
        'upi_id'  : UPI_ID,
        'upi_name': UPI_NAME,
        'qr_data' : qr_data,   # base64 data URL or empty
        'has_qr'  : bool(qr_data),
    })
