...
def chat():
    ...
    # ── Manual ML Commands (work without modals!) ──────────────────
    try:
        # ANOMALY DETECTION
        if any(x in msg_lower for x in ['anomaly', 'anomalies', 'outlier', 'unusual', 'anomali']):
            if agent.df is None:
            ...
            if not num_cols:
                return jsonify({"response": "⚠️ Numeric columns nahi mile dataset mein!"})
            X = df[num_cols].fillna(df[num_cols].median())
            from scipy import stats as _sp
            z = _sp.zscore(X.fillna(0)); z_max = np.abs(z).max(axis=1)
            threshold = float(np.quantile(z_max, 0.95))
            anomaly_mask = np.array(z_max > threshold)
            n_anom = int(anomaly_mask.sum())
            anomaly_rows = df[anomaly_mask][num_cols[:4]].head(10).round(2)
            response = f"""🚨 **Anomaly Detection Results**

📊 Dataset: {len(df)} rows analyzed
🔴 Anomalies Found: **{n_anom}** ({round(n_anom/len(df)*100,1)}%)
🟢 Normal: {len(df)-n_anom}
📏 Method: Z-score (threshold: {threshold:.2f})
📋 Columns Used: {', '.join(num_cols[:5])}

**Top Anomalous Rows:**
{anomaly_rows.to_string()}

💡 **Tip:** Ye rows unusual patterns show karte hain!
Plus → Anomaly Detect modal mein aur options hain."""
            return jsonify({"response": response})

        # DBSCAN CLUSTERING
        if any(x in msg_lower for x in ['dbscan', 'cluster', 'clustering', 'group data', 'segment']):
            if agent.df is None:
                return jsonify({"response": "⚠️ Pehle dataset upload karo!"})
            df = agent.df.copy()
            ...
            num_cols = df.select_dtypes(include='number').columns.tolist()
            if len(num_cols) < 2:
                return jsonify({"response": "⚠️ Clustering ke liye minimum 2 numeric columns chahiye!"})
            ...
            response = f"""🔵 **DBSCAN Clustering Results**

📊 Dataset: {len(df)} rows
🎯 Clusters Found: **{n_clusters}**
🔇 Noise Points: {n_noise}
📋 Features Used: {', '.join(num_cols[:4])}

**Cluster Sizes:**
{chr(10).join([f'  {k}: {v} rows ({round(v/len(df)*100,1)}%)' for k,v in cluster_sizes.items()])}

💡 **Tip:** Plus → DBSCAN Cluster mein visual chart bhi milega!"""
            return jsonify({"response": response})

        # AUTO INSIGHTS
        if any(x in msg_lower for x in ['insight', 'auto insight', 'story', 'summarize', 'summary', 'tell me about']):
            if agent.df is None:
                return jsonify({"response": "⚠️ Pehle dataset upload karo!"})
            df = agent.df.copy()
            ...
            for col in num_cols[:5]:
                s = df[col].dropna()
                stats_lines.append(f"  {col}: mean={s.mean():.2f}, std={s.std():.2f}, min={s.min():.2f}, max={s.max():.2f}")
            
            cat_lines = []
            for col in cat_cols[:3]:
                top = df[col].value_counts().head(3)
                cat_lines.append(f"  {col}: {', '.join([f'{k}({v})' for k,v in top.items()])}")
            ...
            if len(num_cols) >= 2:
                ...
                for i in range(len(num_cols[:6])):
                    for j in range(i+1, len(num_cols[:6])):
                        c = corr.iloc[i,j]
                        if abs(c) > 0.4:
                            corr_lines.append(f"  {num_cols[i]} ↔ {num_cols[j]}: {c:.2f}")

            response = f"""💡 **Auto Dataset Insights**

📊 **Dataset Overview**
  Rows: {df.shape[0]} | Columns: {df.shape[1]}
  Numeric: {len(num_cols)} cols | Categorical: {len(cat_cols)} cols
...
{chr(10).join(cat_lines) if cat_lines else '  No categorical columns'}

🔗 **Strong Correlations** (|r| > 0.4)
{chr(10).join(corr_lines) if corr_lines else '  No strong correlations found'}

💡 **Quick Tips**
  • Type "show dashboard" for visual overview
  • Type "show correlation heatmap" for correlation chart
  • Type "find anomalies" for outlier detection
  • Plus → Auto Insights for AI-generated story"""
            return jsonify({"response": response})

        # PCA ANALYSIS
        if any(x in msg_lower for x in ['pca', 'principal component', 'dimension reduction', 'dimensionality']):
            if agent.df is None:
                return jsonify({"response": "⚠️ Pehle dataset upload karo!"})
            df = agent.df.copy()
            if len(df) > 1000: df = df.sample(1000, random_state=42)
            num_cols = df.select_dtypes('number').columns.tolist()
            if len(num_cols) < 2:
                return jsonify({"response": "⚠️ PCA ke liye minimum 2 numeric columns chahiye!"})
            ...
            pca.fit(Xs)
            var_ratio = [f"PC{i+1}: {v:.1%}" for i,v in enumerate(pca.explained_variance_ratio_)]
            cumvar = np.cumsum(pca.explained_variance_ratio_)
            pc2_var = sum(pca.explained_variance_ratio_[:2])
            response = f"""📐 **PCA Analysis Results**

📊 Dataset: {len(df)} rows, {len(num_cols)} features → {n_comp} components
🎯 First 2 PCs explain: **{pc2_var:.1%}** of variance

**Variance Explained:**
{chr(10).join(['  '+v for v in var_ratio])}

**Cumulative Variance:**
{chr(10).join([f'  Top {i+1} PCs: {v:.1%}' for i,v in enumerate(cumvar)])}

💡 **Tip:** Plus → PCA Analysis mein 2D scatter plot milega!"""
            return jsonify({"response": response})

        # CROSS VALIDATION
        if any(x in msg_lower for x in ['cross val', 'cross-val', 'cv score', 'k-fold', 'kfold']):
            return jsonify({"response": """📊 **Cross Validation**

Yeh feature Plus menu → ✅ Cross Validation mein available hai.

**Ya seedha type karo:**
`predict [target_column]` — AutoML + CV automatically run hoga!

**Example:**
`predict Is_Fraud` — Fraud prediction with 5-fold CV"""})
