...
# ✅ Logging Setup (Detailed errors ke liye)
...
# ✅ FIX: Render compatible upload folder (/tmp is writable)
UPLOAD_DIR = '/tmp/uploads' if os.path.exists('/tmp') else './uploads'
os.makedirs(UPLOAD_DIR, exist_ok=True)
app.config['UPLOAD_FOLDER'] = UPLOAD_DIR
app.config['MAX_CONTENT_LENGTH'] = 16 * 1024 * 1024  # Limit badha di 16MB tak
    ...
    if file and file.filename != '':
        ...
        try:
            ...
            if not os.path.exists(full_path):
                logger.error("❌ File save failed: File not found after saving")
                return jsonify({"error": "File save failed: Permission denied?"}), 500
            
            logger.info(f"✅ File saved successfully: {filename}")
            
            # Try loading data
            logger.info(f"🤖 Loading data into agent...")
            result = agent.load_data(full_path)
            
            logger.info(f"✅ Data loaded successfully: {result[:50]}...")
            return jsonify({"message": f"✅ Uploaded: {filename}\n{result}"}), 200
            
        except UnicodeDecodeError as e:
            logger.error(f"❌ Encoding Error: {str(e)}")
            return jsonify({"error": f"CSV Encoding Error: File shayad UTF-8 mein nahi hai. Excel se 'CSV UTF-8' format mein save karo. Detail: {str(e)[:100]}"}), 500
        except Exception as e:
            logger.error(f"❌ Upload Exception: {str(e)}")
            return jsonify({"error": f"Server error: {str(e)[:150]}"}), 500
