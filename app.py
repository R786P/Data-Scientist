...
def shap_explain():
    try:
        ...
        if isinstance(shap_vals, list):
            shap_vals = _snp.array(shap_vals)
            if shap_vals.ndim == 3:
                shap_vals = shap_vals.mean(axis=0)  # avg across classes
            else:
        ...
        fig, axes = plt.subplots(1, 2, figsize=(9, 4))
        mean_shap = _snp.abs(shap_vals).mean(axis=0)
        feat_imp = dict(zip(X.columns, mean_shap.tolist()))
