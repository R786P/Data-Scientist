...
try:
    ...
    logger.warning("resend package nahi mila — pip install resend")

FROM_EMAIL = os.getenv("FROM_EMAIL", "DS Agent <onboarding@resend.dev>")
...
def send_otp_email(to_email: str, otp: str, username: str) -> bool:
    ...
    try:
        resend.Emails.send({
            "from": FROM_EMAIL,
            "to": [to_email],
            "subject": "🔐 DS Agent — Email Verify Karo",
            "html": f"""<!DOCTYPE html>
<html><head><meta charset="UTF-8"><style>
body{{font-family:'Segoe UI',sans-serif;background:#f6f7fb;margin:0;padding:20px}}
.card{{max-width:480px;margin:0 auto;background:white;border-radius:20px;box-shadow:0 4px 24px rgba(91,94,244,.12);overflow:hidden}}
.hdr{{background:linear-gradient(135deg,#5b5ef4,#8b5cf6);padding:32px 28px;text-align:center}}
.hdr h1{{color:white;margin:0;font-size:26px}}.hdr p{{color:rgba(255,255,255,.85);margin:8px 0 0;font-size:14px}}
.body{{padding:32px 28px}}
.otp-box{{background:linear-gradient(135deg,#f0f0ff,#f5f0ff);border:2px solid #5b5ef4;border-radius:16px;padding:24px;text-align:center;margin:24px 0}}
...
.warn{{background:#fff8e1;border-left:4px solid #f59e0b;padding:12px 16px;border-radius:8px;font-size:13px;color:#78350f;margin-top:20px}}
.footer{{padding:20px 28px;background:#f8f9fc;text-align:center;font-size:11px;color:#aaa}}
</style></head><body>
<div class="card">
  <div class="hdr"><h1>🤖 DS Agent</h1><p>AI-powered Data Science Platform</p></div>
  <div class="body">
    <p style="font-size:16px;color:#1a1a2e">Namaste <b>{username}</b>! 👋</p>
    <p style="color:#666;font-size:14px">Tumhara account verify karne ke liye neeche diya OTP use karo:</p>
    <div class="otp-box">
      <div class="otp">{otp}</div>
      <div class="expire">⏰ 10 minutes mein expire ho jayega</div>
    </div>
    <p style="color:#666;font-size:13px">Ye OTP sirf DS Agent signup ke liye hai.<br>Kisi ke saath share mat karo!</p>
    <div class="warn">⚠️ Agar tumne signup nahi kiya toh is email ko ignore karo.</div>
  </div>
  <div class="footer">© 2025 DS Agent</div>
</div></body></html>"""
        })
        return True
    except Exception as e:
...
def register():
    if request.method == 'POST':
        ...
        try:
            ...
            return render_template('register.html', error="System error! Dobara try karo.")
        finally:
            db.close()
