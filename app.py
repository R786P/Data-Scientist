...
# ═══════════════════════════════════════════════════════════════
# 🎯 PRINCIPAL / DIRECTOR LEVEL DS — 9 ROLE AREAS
# ═══════════════════════════════════════════════════════════════
...
def _get_df_context():
    ...
    shape = df.shape
    num_cols = df.select_dtypes('number').columns.tolist()[:10]
    cat_cols = df.select_dtypes('object').columns.tolist()[:10]
    sample = df.head(3).to_dict(orient='records')
    return f"""Dataset: {shape[0]} rows × {shape[1]} columns
Columns: {cols}
Numeric: {num_cols}
...
def principal_business_framing():
    """Business Problem → Measurable Data Objectives"""
    try:
        data = request.get_json()
        business_goal = data.get('business_goal', '')
        industry = data.get('industry', 'general')
        df_ctx = _get_df_context()
        prompt = f"""You are a Principal Data Scientist / Director of Data Science.

Business Goal: {business_goal}
Industry: {industry}
Dataset Context: {df_ctx}

Convert this business problem into a complete data science framework:
1. **Measurable Objectives** — specific KPIs and success metrics
2. **Data Requirements** — what data is needed, what's available
3. **ML Problem Formulation** — classification/regression/clustering etc.
4. **Success Criteria** — how to measure if DS solution solved the business problem
5. **ROI Estimation** — expected business impact
6. **Risk & Assumptions** — what could go wrong

Be specific, practical, production-focused. Use real column names from dataset if available.
Respond in Hinglish."""
...
Current Dataset: {df_ctx}

Design a complete scalable data architecture:
1. **Ingestion Layer** — sources, batch vs streaming, tools
2. **Storage Layer** — data lake, warehouse, feature store design
3. **Processing Pipeline** — ETL/ELT, transformation logic
4. **Feature Engineering Pipeline** — feature computation, versioning
5. **Model Serving Layer** — real-time vs batch inference
6. **Monitoring & Observability** — data drift, model drift, alerts
7. **Tech Stack Recommendation** — specific tools for this scale
8. **Cost Estimation** — rough cloud cost estimate

Be specific to the dataset and use case. Respond in Hinglish."""
...
Create a complete model development & optimization strategy:
1. **Baseline Model** — simplest model to beat, expected baseline score
2. **Model Candidates** — 3-5 recommended models with justification
3. **Feature Engineering Strategy** — specific transformations for this data
4. **Class Imbalance / Data Issues** — handling strategy
5. **Hyperparameter Optimization** — search strategy, key params
6. **Ensembling Strategy** — stacking/blending approach
7. **Production Optimization** — latency, memory, quantization
8. **Validation Strategy** — cross-validation, holdout, time-based split
9. **Expected Performance Range** — realistic accuracy/AUC targets
...
Design a rigorous experimentation framework:
1. **Hypothesis Formulation** — H0, H1, directional vs non-directional
2. **Sample Size Calculation** — power analysis, MDE, significance level
3. **Experiment Design** — control/treatment split, randomization unit
4. **Guardrail Metrics** — what NOT to break
5. **Statistical Test Selection** — t-test, Mann-Whitney, Chi-square etc.
6. **Duration Estimation** — how long to run
7. **Novelty & Seasonality Effects** — handling
8. **Multi-variate Testing** — if multiple variants needed
9. **Decision Framework** — when to ship, rollback criteria
10. **Python Code** — sample size calculation + analysis code

Be practical and specific. Respond in Hinglish."""
...
def principal_cross_functional():
    ...
    try:
        ...
        teams = data.get('teams', 'Product, Engineering, Business')
        challenge = data.get('challenge', '')
        df_ctx = _get_df_context()
        prompt = f"""You are a Principal Data Scientist leading cross-functional DS projects.
...
Create a cross-functional collaboration plan:
1. **Stakeholder Map** — who needs what, decision makers
2. **Data Contract** — API specs, data format, SLAs
3. **Communication Cadence** — meetings, updates, escalation
4. **DS ↔ Engineering Handoff** — model packaging, deployment requirements
5. **DS ↔ Product Alignment** — feature flags, rollout plan
6. **DS ↔ Business Translation** — KPI mapping, business narrative
7. **Conflict Resolution** — when priorities clash
8. **Documentation Standards** — what to document for each team

Be practical. Respond in Hinglish."""
...
def principal_mentorship():
    ...
    try:
        ...
        mentee_level = data.get('mentee_level', 'junior')
        code_or_approach = data.get('code_or_approach', '')
        df_ctx = _get_df_context()
        prompt = f"""You are a Principal Data Scientist mentoring your team.

Situation: {situation}
Mentee Level: {mentee_level}
Code/Approach to Review: {code_or_approach}
Project Dataset: {df_ctx}

Provide structured mentorship guidance:
1. **What's Good** — specific praise on correct approaches
2. **Critical Issues** — bugs, anti-patterns, performance problems
3. **Best Practice Violations** — industry standards not followed
4. **Improved Approach** — better way to solve this
5. **Learning Resources** — specific papers, docs, courses
6. **Next Challenge** — what to learn next for growth
7. **Production Readiness** — is this deployable? what's missing?

If code provided, give corrected version. Be encouraging but honest. Respond in Hinglish."""
...
def principal_governance():
    ...
    try:
        data = request.get_json()
        use_case = data.get('use_case', '')
        regulation = data.get('regulation', 'GDPR, HIPAA')
        df_ctx = _get_df_context()
...
Create a comprehensive data governance & ethics framework:
1. **PII/Sensitive Data Audit** — identify sensitive columns in dataset
2. **Bias Analysis** — potential bias sources, protected attributes
3. **Fairness Metrics** — demographic parity, equalized odds, calibration
4. **Privacy Compliance** — GDPR/HIPAA requirements for this use case
5. **Data Lineage** — tracking data origin and transformations
6. **Model Explainability Requirements** — regulatory requirements
7. **Consent & Data Rights** — user rights, deletion requests
8. **Audit Trail** — logging requirements
9. **Risk Assessment** — high/medium/low risk categorization
10. **Mitigation Plan** — specific actions for each risk

Be specific to the dataset columns. Respond in Hinglish."""

        result = get_groq_response("You are a Principal Data Scientist expert in data governance and ethics.", prompt, max_tokens=1500)
...
def principal_communication():
    ...
    try:
        data = request.get_json()
        audience = data.get('audience', 'C-suite')
        finding = data.get('finding', '')
        action = data.get('action', '')
        df_ctx = _get_df_context()
        prompt = f"""You are a Principal Data Scientist presenting to {audience}.

Key Finding: {finding}
Recommended Action: {action}
Dataset Context: {df_ctx}

Create a compelling non-technical communication package:
1. **Executive Summary** (3 sentences max) — the finding + so what + do what
2. **Business Impact Narrative** — translate technical finding to money/risk/growth
3. **Visualization Recommendations** — what charts to show, why
4. **Objection Handling** — anticipated questions + answers
5. **Decision Framework** — clear YES/NO recommendation with confidence level
6. **Risk of Inaction** — what happens if we ignore this finding
7. **Email Draft** — ready-to-send stakeholder email
8. **Presentation Outline** — 5-slide structure

Be persuasive and business-focused. Respond in Hinglish."""

        result = get_groq_response("You are a Principal Data Scientist with exceptional communication skills.", prompt, max_tokens=1500)
...
def principal_vision_strategy():
    """Long-Term Vision & Data Strategy"""
    try:
        data = request.get_json()
        org_type = data.get('org_type', 'healthcare startup')
        current_maturity = data.get('current_maturity', 'beginner')
        goals = data.get('goals', '')
        df_ctx = _get_df_context()
        prompt = f"""You are a Director of Data Science / Principal DS setting long-term strategy.

Organization Type: {org_type}
Current DS Maturity: {current_maturity}
Goals: {goals}
Current Data: {df_ctx}

Design a comprehensive 3-year data science roadmap:
1. **Current State Assessment** — maturity model score, gaps
2. **Year 1: Foundation** — quick wins, infrastructure, hiring
3. **Year 2: Scale** — platform, automation, advanced models
4. **Year 3: Innovation** — AI-first products, competitive advantage
5. **Team Structure** — roles needed at each stage
6. **Technology Roadmap** — tools to adopt/retire by year
7. **Data Products Roadmap** — internal tools, customer-facing AI
8. **Success Metrics** — OKRs for DS org
9. **Budget Estimation** — rough annual DS investment
10. **Quick Wins (30 days)** — 3 things to do immediately

Be strategic and realistic. Respond in Hinglish."""

        result = get_groq_response("You are a Director of Data Science setting organizational strategy.", prompt, max_tokens=1800)
        return jsonify({'success': True, 'result': result, 'message': '✅ Strategy Roadmap Ready!'})
    except Exception as e:
        return jsonify({'error': str(e)}), 500
