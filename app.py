...
def call_hf_api(url, endpoint, payload):
    ...
    hf = os.environ.get('HF_SPACE_URL', '').strip()
    if url == hf:
        # fn_index map — matches order of .click() in hf_space/app.py
        # 0=ping, 1=run(DL), 2=xray, 3=ecg, 4=skin, 5=ocr
        ...
        try:
            r = req.post(url + '/run/predict', json=gradio_payload, timeout=180)
            r.raise_for_status()
            resp = r.json()
            print(f"[HF] raw response keys: {list(resp.keys()) if isinstance(resp, dict) else type(resp)}")
            
            if 'data' in resp and len(resp['data']) > 0:
                raw = resp['data'][0]
                print(f"[HF] raw data[0] type: {type(raw)}, value[:200]: {str(raw)[:200]}")
                # data[0] could be string (JSON) or already a dict
