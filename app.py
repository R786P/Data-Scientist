...
def colab_status():
    ...
    if not colab_url:
        return jsonify({'connected': False, 'message': 'Colab URL set nahi hai'})
    try:
        import requests as req
        r = req.get(colab_url + '/ping', timeout=5)
        if r.status_code == 200:
            return jsonify({'connected': True, 'url': colab_url, 'message': 'Colab connected!'})
    except:
        pass
    return jsonify({'connected': False, 'message': 'Colab offline hai — notebook open karo'})
...
def colab_deep_learning():
    """Run Deep Learning on Colab GPU"""
    colab_url = os.environ.get('COLAB_URL', '').strip()
    if not colab_url:
        return jsonify({'error': 'COLAB_URL set nahi — Render env mein add karo'}), 400
    try:
        import requests as req
        data = request.get_json()
        ...
        r = req.post(colab_url + '/run', json=payload, timeout=300)
