...
def kpi_dashboard():
    try:
        ...
        kpi_defs = db.query(UserChart).filter_by(
            user_id=get_user_id(), chart_type='kpi_definition'
        ).all()
        db.close()

        if not kpi_defs:
            return jsonify({'error': 'Koi KPI define nahi hai! Pehle ek KPI add karo.'}), 400
        if agent.df is None:
            return jsonify({'error': 'Dataset upload karo pehle! CSV/Excel file drag karo.'}), 400
        ...
        # Visualization
        fig, axes = _kpi_plt.subplots(1, 2, figsize=(9, 4))
        ...
        _kpi_plt.setp(axes[0].xaxis.get_majorticklabels(), rotation=30, ha='right')
        ...
        if targets and len(targets) == len(actuals):
            ...
            _kpi_plt.setp(axes[1].xaxis.get_majorticklabels(), rotation=30, ha='right')

        on_track = sum(1 for k in kpi_results if k['status'] == 'ON_TRACK')
        _kpi_plt.suptitle(f'KPI Dashboard — {on_track}/{len(kpi_results)} On Track',
                    fontweight='bold', fontsize=13, color='#1e3a5f')
