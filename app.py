...
def auto_ml():
    ...
    try:
        ...
        if best_model and hasattr(best_model[1], 'feature_importances_'):
            ...
            plt.tight_layout()
            buf = _io.BytesIO(); plt.savefig(buf, format='png', dpi=80); plt.close('all')
...
def hc_dashboard():
    ...
    try:
        ...
        plt.tight_layout()
        buf = _io.BytesIO(); plt.savefig(buf, format='png', dpi=80, bbox_inches='tight'); plt.close('all')
...
def shap_explain():
    try:
        ...
        buf = _io.BytesIO(); plt.savefig(buf, format='png', dpi=80, bbox_inches='tight')
...
def anomaly_detection():
    try:
        ...
        if method == 'isolation_forest':
            clf = IsolationForest(contamination=contamination, random_state=42, n_estimators=20)
            preds = clf.fit_predict(Xs)
            scores = clf.score_samples(Xs)
        elif method == 'lof':
            clf = LocalOutlierFactor(contamination=contamination, n_neighbors=5)
            preds = clf.fit_predict(Xs)
        ...
        axes[1].set_title(f'Result: {anomaly_pct}% Anomalies', fontweight='bold')
...
def generate_pptx():
    try:
        ...
        if len(num_cols_list) >= 3:
            ...
            plt.tight_layout()
            buf2 = _io.BytesIO(); plt.savefig(buf2, format='png', dpi=80, bbox_inches='tight',
        ...
        if target and target in agent.df.columns:
            ...
            fig.patch.set_facecolor('#'+th['bg'])
            plt.tight_layout()
            buf3 = _io.BytesIO(); plt.savefig(buf3, format='png', dpi=80, bbox_inches='tight',
...
def ml_dbscan():
    ...
    try:
        ...
        _dbdf = agent.df.copy()
        if len(_dbdf) > 2000:
            _dbdf = _dbdf.sample(n=800, random_state=42).reset_index(drop=True)

        num_cols = _dbdf.select_dtypes('number').columns.tolist()
        if len(num_cols) < 2:
            return jsonify({'error': 'Minimum 2 numeric columns chahiye!'}), 400
...
def ml_confusion_matrix():
    ...
    try:
        ...
        if n_classes == 2:
            fig, axes = _plt.subplots(1, 3, figsize=(10, 4))
        else:
...
def health_full_check():
    ...
    try:
        ...
        def check_matplotlib():
            import matplotlib; matplotlib.use('Agg')
            import matplotlib.pyplot as _plt, io as _io
            fig, ax = _plt.subplots(); ax.plot([1,2,3])
            buf = _io.BytesIO(); _plt.savefig(buf,format='png'); _plt.close('all')
        ...
        def check_seaborn():
            ...
            _sns.scatterplot(data=df,x='x',y='y'); buf = _io.BytesIO()
            _plt.savefig(buf,format='png'); _plt.close('all')
...
def ml_engineer_model_profile():
    ...
    try:
        import time as _t, base64, io as _io
        import matplotlib; matplotlib.use('Agg')
        import matplotlib.pyplot as plt
        import numpy as np
        from sklearn.model_selection import train_test_split
...
def video_generate_mp4():
    ...
    try:
        import base64, io as _io, os, textwrap, time as _t
        import numpy as np
    ...
    try:
        ...
        # Priority 2: Uploaded file in static/
            ...
            with open(_os.path.join(_os.path.dirname(__file__), 'static', 'qr_code.png'), 'rb') as f:
                qr_data = 'data:image/png;base64,' + _b64.b64encode(f.read()).decode()
