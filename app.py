    ...
    try:
        ...
        if platform == 'youtube':
            ...
            try:
                ...
                for pat in [r'"numLikes":(\d+)', r'reactions.*?(\d+)', r'(\d+)\s*reaction']:
                    m = _re2.search(pat, html_li, _re2.IGNORECASE)
                    if m: reactions = int(m.group(1)); break

                for pat in [r'"numComments":(\d+)', r'comments.*?(\d+)', r'(\d+)\s*comment']:
                    m = _re2.search(pat, html_li, _re2.IGNORECASE)
                    if m: comments = int(m.group(1)); break

                for pat in [r'"numShares":(\d+)', r'reposts.*?(\d+)', r'(\d+)\s*repost']:
                    m = _re2.search(pat, html_li, _re2.IGNORECASE)
                    if m: reposts = int(m.group(1)); break

                # Extract post text
                text_preview = ''
                for pat in [r'"commentary":\{"text":"([^"]{20,300})"', r'<p[^>]*>([^<]{20,200})</p>']:
                    m = _re2.search(pat, html_li)
                    if m: text_preview = m.group(1)[:200]; break
