...
- Ready to publish as-is"""
        ...
        # Save to DB — store all in image_data as JSON
        import json as _j2
        db = SessionLocal()
        save_data = _j2.dumps({
        ...
        entry = UserChart(
            ...
            chart_title=f'{type_label} | {topic[:60]}',
            image_data=save_data
        )
...
- PLATFORMS TO REPURPOSE ON (3 suggestions)"""
        ...
        entry = UserChart(
            ...
            image_data=_j3.dumps({
                'category': category,
                'content_type': content_type,
                'content': finalized[:6000]
            })
        )
...
def content_history():
    try:
        ...
        history = []
        for e in entries:
            try: meta = _json.loads(e.image_data or '{}')
            except: meta = {}
            content_preview = meta.get('content', '')
            history.append({
                'id': e.id,
                'title': e.chart_title or '',
                'type': e.chart_type,
