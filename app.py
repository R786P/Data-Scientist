...
def anomaly_detection():
    try:
        ...
        import base64, warnings as _w; _w.filterwarnings('ignore')
        ...
        if method == 'isolation_forest':
            clf = IsolationForest(contamination=contamination, random_state=42, n_estimators=50, n_jobs=-1)
            preds = clf.fit_predict(Xs)
            scores = clf.score_samples(Xs)
        elif method == 'lof':
            clf = LocalOutlierFactor(contamination=contamination, n_neighbors=10, n_jobs=-1)
            preds = clf.fit_predict(Xs)
        ...
        anomaly_scores = [s for s,m in zip(scores, anomaly_mask) if m]; axes[1].axvline(float(max(anomaly_scores)) if anomaly_scores else 0, color='#ef4444', linestyle='--', linewidth=2, label='Anomaly threshold')
...
def ml_dbscan():
    ...
    try:
        ...
        _dbdf = agent.df.copy()
        if len(_dbdf) > 2000:
            _dbdf = _dbdf.sample(n=2000, random_state=42).reset_index(drop=True)
    ...
    try:
        ...
        if task == 'classification':
            ...
            models = {
                ...
                'Extra Trees':         ExtraTreesClassifier(n_estimators=100, random_state=42) if False else __import__('sklearn.ensemble', fromlist=['ExtraTreesRegressor']).ExtraTreesRegressor(n_estimators=100, random_state=42),
                'Linear Regression':   LinearRegression(),
