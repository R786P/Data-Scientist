...
from core.database import (engine, Base, SessionLocal, UserQuery, UserDataset,
...
def list_charts():
    import glob, json
    charts = sorted(glob.glob('/tmp/plot_*.png'))
    return jsonify({"charts": [f"/plot/{i+1}" for i,_ in enumerate(charts)], "total": len(charts)})

@app.route('/plot/<int:num>')
@login_required  
def plot_numbered(num):
    path = f'/tmp/plot_{num}.png'
    if not os.path.exists(path):
        return jsonify({'error': f'Chart {num} nahi mila!'}), 404
...
def clear_charts():
    import glob
    for f in glob.glob('/tmp/plot_*.png'):
        os.remove(f)
