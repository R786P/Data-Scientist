...
def auto_report():
    ...
    try:
        ...
        <style>
        body{{font-family:'Segoe UI',sans-serif;background:#f6f7fb;margin:0;padding:20px;color:#1a1a2e}}
        ...
        .header{{background:linear-gradient(135deg,#5b5ef4,#8b5cf6);color:white;padding:28px 32px}}
        .header h1{{margin:0;font-size:24px}} .header p{{margin:6px 0 0;opacity:0.8;font-size:13px}}
        .section{{padding:20px 32px;border-bottom:1px solid #f0f0f8}}
        .section h2{{font-size:16px;font-weight:800;color:#5b5ef4;margin:0 0 12px}}
        table{{width:100%;border-collapse:collapse;font-size:12px}}
        th{{background:#f0f0f8;padding:8px 10px;text-align:left;font-weight:700;color:#555}}
        ...
        .stat-box .val{{font-size:20px;font-weight:800;color:#5b5ef4}}
        ...
        <div class='header'><h1>📊 Auto Data Report</h1>
        <p>Generated: {datetime.datetime.now().strftime('%d %b %Y, %H:%M')} &nbsp;|&nbsp; {len(df):,} rows × {len(df.columns)} columns</p></div>"""
...
def colab_status():
    ...
    if hf_url:
        try:
            r = req.get(hf_url + '/ping', timeout=5)
            if r.status_code == 200:
                return jsonify({'connected': True, 'url': hf_url, 'source': 'HuggingFace', 'message': 'HuggingFace ML Server connected! (Permanent)'})
        except: pass

    # Fallback to Colab
    if colab_url:
        try:
            r = req.get(colab_url + '/ping', timeout=5)
            if r.status_code == 200:
                return jsonify({'connected': True, 'url': colab_url, 'source': 'Colab', 'message': 'Colab connected! (Backup)'})
        except: pass

    return jsonify({'connected': False, 'message': 'Dono offline! HF_SPACE_URL ya COLAB_URL set karo.'})
...
def colab_deep_learning():
    ...
    if hf_url:
        try:
            req.get(hf_url + '/ping', timeout=4)
            server_url = hf_url
        except: pass
    if not server_url and colab_url:
        try:
            req.get(colab_url + '/ping', timeout=4)
            server_url = colab_url
        except: pass
    if not server_url:
        return jsonify({'error': 'ML Server offline! HuggingFace Space ya Colab notebook start karo.'}), 400
    try:
        import requests as req
        data = request.get_json()
        ...
        r = req.post(server_url + '/run', json=payload, timeout=300)
...
def medical_xray():
    ...
    try:
        import requests as req
        data = request.get_json()
        r = req.post(url + '/xray', json=data, timeout=60)
        return jsonify(r.json())
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def medical_ecg():
    ...
    try:
        import requests as req
        data = request.get_json()
        r = req.post(url + '/ecg', json=data, timeout=60)
        return jsonify(r.json())
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def medical_skin():
    ...
    try:
        import requests as req
        data = request.get_json()
        r = req.post(url + '/skin', json=data, timeout=60)
        return jsonify(r.json())
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def medical_ocr():
    ...
    try:
        import requests as req
        data = request.get_json()
        r = req.post(url + '/ocr', json=data, timeout=60)
        return jsonify(r.json())
    except Exception as e:
        return jsonify({'error': str(e)}), 500
