...
def call_hf_api(url, endpoint, payload):
    ...
    hf = os.environ.get('HF_SPACE_URL', '').strip()
    if url == hf:
        # Map endpoint to Gradio function name
        fn_name_map = {
            '/run': 'run_fn',
            '/xray': 'xray_fn',
            '/ecg': 'ecg_fn',
        ...
        fn_name = fn_name_map.get(endpoint, 'run_fn')
        # Use api_name (more reliable than fn_index)
        gradio_payload = {'data': [_json.dumps(payload)]}
        r = req.post(url + '/call/' + fn_name, json=gradio_payload, timeout=10)
        if r.status_code == 200:
            ...
            if event_id:
                ...
                import time
                for _ in range(60):
                    result_r = req.get(url + '/call/' + fn_name + '/' + event_id, timeout=120)
                    lines = result_r.text.strip().split('\n')
                    for line in lines:
                        if line.startswith('data:'):
                            ...
                            try:
                                ...
                                if isinstance(data, list) and len(data) > 0:
                                    return _json.loads(data[0])
                            except: pass
                    time.sleep(2)
        # Fallback: try old /run/predict
        gradio_payload2 = {'data': [_json.dumps(payload)], 'fn_index': 0}
        r2 = req.post(url + '/run/predict', json=gradio_payload2, timeout=120)
        result = r2.json()
        if 'data' in result:
            try: return _json.loads(result['data'][0])
