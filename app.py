...
def colab_deep_learning():
    """Deep Learning guide + code using Groq AI"""
    try:
        ...
        if agent.df is not None:
            ...
            num_cols = df.select_dtypes('number').columns.tolist()
            cat_cols = df.select_dtypes('object').columns.tolist()
            df_info = f"Dataset: {df.shape[0]} rows, {df.shape[1]} cols. Numeric: {num_cols[:5]}. Text: {cat_cols[:3]}."
        
        prompt = f"""You are a Deep Learning expert. Generate practical PyTorch/TensorFlow code.
...
Generate:
1. Architecture recommendation (which DL model, why)
2. Complete working Python code (PyTorch preferred)
3. Training loop with early stopping
4. Evaluation metrics
5. Improvement tips

Keep code practical and runnable. Use Hinglish for explanations."""

        result = get_groq_response(
            "You are a Deep Learning expert. Generate clean, working PyTorch code.",
        ...
        return jsonify({
            ...
            'result': result,
            'task': task,
            'message': f'✅ Deep Learning guide ready for {task} task!'
        })
    except Exception as e:
        return jsonify({'error': str(e)}), 500
