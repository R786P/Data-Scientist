...
def save_model_version():
    try:
        ...
        chart = UserChart(
            ...
            chart_data=model_b64,
            title=f"v{version_num} — {model_name}",
            description=f"target={target}|task={task}|score={score:.4f}|features={len(X.columns)}|rows={len(df)}|notes={notes}|cv_mean={cv_scores.mean():.4f}|cv_std={cv_scores.std():.4f}"
        )
...
def registry_save():
    try:
        ...
        reg = UserChart(
            ...
            title=f"{model_name} | {model_type} | score={score:.4f}",
            description=_json.dumps(metadata),
            chart_data=model_b64
        )
...
def deploy_model_api():
    try:
        ...
        if existing:
            existing.chart_data = model_b64; existing.description = meta; db.commit()
            deploy_id = existing.id
        else:
            dep = UserChart(user_id=current_user.id, chart_type='mlops_deployed',
                          title=model_name, description=meta, chart_data=model_b64)
...
def schedule_report():
    try:
        ...
        sched = UserChart(
            ...
            title=f'{report_name} — {frequency}',
            description=schedule_data,
            chart_data=''
        )
...
def bdm_pipeline_add():
    try:
        ...
        entry = UserChart(
            ...
            title=f"{lead['client_name']} — {lead['stage']}",
            description=_json.dumps(lead),
            chart_data=''
        )
...
- Alternative shorter version (for WhatsApp/SMS)"""
        ...
        e = UserChart(
            ...
            title=f'Outreach — {prospect_company} ({email_type})',
            description=_json.dumps({'company': prospect_company, 'role': prospect_role, 'type': email_type}),
            chart_data=email_content[:3000]
        )
...
4. One-line WhatsApp follow-up version"""
        ...
        e = UserChart(
            user_id=current_user.id,
            chart_type='ea_email',
            title=f'Email to {to_name} — {subject_hint[:30]}',
            description=_json.dumps({'to': to_name, 'purpose': email_purpose, 'tone': tone}),
            chart_data=email_draft[:3000]
        )
        db.add(e); db.commit(); db.close()
...
Format professionally. Be specific about action items."""
        ...
        e = UserChart(
            ...
            title=f'Meeting Notes — {date} ({meeting_type})',
            description=_json.dumps({'type': meeting_type, 'date': date, 'attendees': attendees}),
            chart_data=summary[:5000]
        )
        db.add(e); db.commit(); db.close()
...
def ea_reminder_add():
    try:
        ...
        r = UserChart(
            ...
            description=_json.dumps(reminder),
            chart_data=''
        )
...
- Ready to publish as-is"""
        ...
        entry = UserChart(
            user_id=current_user.id,
            chart_type='content_generated',
            chart_title=f'{type_label} — {topic[:50]}',
            description=_json.dumps({
                'category': category, 'content_type': content_type,
                'topic': topic, 'tone': tone, 'length': length,
                'keywords': keywords, 'language': language,
            }),
            chart_data=content_text[:8000]
        )
        db.add(entry); db.commit(); cid = entry.id; db.close()
...
- PLATFORMS TO REPURPOSE ON (3 suggestions)"""
        ...
        entry = UserChart(
            user_id=current_user.id,
            chart_type='content_finalized',
            chart_title=f'FINAL: {type_label} — {category}',
            description=_json.dumps({'category': category, 'content_type': content_type}),
            chart_data=finalized[:8000]
        )
...
def content_history():
    try:
        import json as _json
        db = SessionLocal()
        entries = db.query(UserChart).filter(
            UserChart.user_id == current_user.id,
            UserChart.chart_type.in_(['content_generated', 'content_finalized'])
        ).order_by(UserChart.created_at.desc()).limit(20).all()
        db.close()

        history = []
        for e in entries:
            try: meta = _json.loads(e.description)
            except: meta = {}
            history.append({
                'id': e.id,
                'title': e.title,
                ...
                'preview': e.chart_data[:200] if e.chart_data else '',
                'created_at': e.created_at.strftime('%d %b %Y %I:%M %p'),
                'word_count': len(e.chart_data.split()) if e.chart_data else 0,
            })
