...
def call_hf_api(url, endpoint, payload):
    """Call HuggingFace Gradio API or Colab Flask API"""
    import requests as req, json as _json
    hf = os.environ.get('HF_SPACE_URL', '').strip()
    if url == hf:
        fn_index_map = {'/run': 1, '/xray': 2, '/ecg': 3, '/skin': 4, '/ocr': 5}
        fn_index = fn_index_map.get(endpoint, 1)
        gradio_payload = {'data': [_json.dumps(payload)], 'fn_index': fn_index}
        r = req.post(url + '/run/predict', json=gradio_payload, timeout=120)
        result = r.json()
        if 'data' in result:
            return _json.loads(result['data'][0])
        return result
    else:
        r = req.post(url + endpoint, json=payload, timeout=120)
        return r.json()
