...
def call_hf_api(url, endpoint, payload):
    ...
    import requests as req, json as _json, time as _time, uuid as _uuid
    hf = os.environ.get('HF_SPACE_URL', '').strip()
    if url == hf:
        # fn_index: 0=ping, 1=run(DL), 2=xray, 3=ecg, 4=skin, 5=ocr
        fn_index_map = {'/run': 1, '/xray': 2, '/ecg': 3, '/skin': 4, '/ocr': 5}
        fn_index = fn_index_map.get(endpoint, 1)
        session_hash = _uuid.uuid4().hex[:8]
        ...
        try:
            ...
            r = req.post(url + '/queue/join', json=join_payload, timeout=30)
            r.raise_for_status()
            print(f"[HF] queue/join status: {r.status_code}")

            # Step 2: Poll /queue/data SSE stream for result
            ...
            while _time.time() < deadline:
                ...
                for raw_line in stream.iter_lines():
                    ...
                    try:
                        ...
                        print(f"[HF] SSE msg type: {msg_type}")
                        ...
                        elif msg_type in ('queue_full', 'heartbeat'):
                            continue

                    except Exception as parse_err:
                        print(f"[HF] parse error: {parse_err}")
                        continue
                _time.sleep(1)

            return {'error': 'HuggingFace timeout — 3 min mein response nahi aaya'}

        except Exception as e:
            print(f"[HF] call_hf_api error: {e}")
            return {'error': f'HuggingFace API error: {str(e)}'}
    else:
